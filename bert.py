...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    ##softmax sum over seq_len
    bs, num_attention_heads, seq_len, attention_head_size=query.shape
    softmax=nn.Softmax(dim=-1)
    score=torch.matmul(query, key.transpose(-2, -1))
    # using the mask in the function attention to convert all paddings to a large negative value, so that they turn into zeros after softmax.
    score=score/math.sqrt(attention_head_size)+attention_mask
    
    # normalize the scores (sum over attention_head_size)
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    transform_output=dense_layer(output)
    drop_out_output=dropout(transform_output)
    final_norm=ln_layer(drop_out_output+input)
    return final_norm
    # todo
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    added_attention=self.add_norm(hidden_states,attention,self.attention_dense,self.attention_dropout,self.attention_layer_norm)
    # feed forward
    interm_feed=self.interm_dense(added_attention)
    interm_output=self.interm_af(interm_feed)
    #interm_output=self.out_dense(interm_mid)
...
class BertModel(BertPreTrainedModel):
  ...
  def __init__(self, config):
    ...
    ##It add a buffer for the model_state that is not a parameter  
  ...
  def embed(self, input_ids):
    ...
    embeds = word_embds + tk_type_embeds + pos_embeds
